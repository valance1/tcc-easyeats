...
if(isset($_POST['email']) || isset($_POST['senha'])){
  if(strlen($_POST['email']) ==0){
    ...
    $query = $mysqli->query($sqlCode) or die('Falha no codigo SQL' . $mysqli->error );
    // VERIFICANDO BUSCAS
  ...
  // CASO NÃO ENCONTRE PESSOA FÍSICA, BUSQUE POR EMPRESA 
    if( $sql_query->num_rows == 1){
      $usuario = $sql_query->fetch_assoc();
      if(!isset($_SESSION)){
        session_start();
